...
(** Get insert scored links input data *)
let get_insert_scored_links_data json_content =
  try
    let data = to_list (member "data" json_content) in
    let aux json =
      let origin_uri = member "origin_uri" json in
      let target_uri = member "target_uri" json in
      let tags_uri = member "tags_uri" json in
      let score = member "score" json in
      (to_string origin_uri, to_string target_uri,
       List.map to_string (to_list tags_uri), to_int score)
    in
    List.map aux data
  with
  | _ -> raise (Pum_exc (return_not_found, "Bad insert_scored_links format"))
